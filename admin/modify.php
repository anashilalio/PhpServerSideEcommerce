...
if(isset($_POST) && !empty($_POST)) {
  ...
  $stmt = $db->prepare("UPDATE products set name= ? , description = ?, price = ?  , categorie = ?  , images = ?  , autheur = ?  , dat = ?  ,createdDate = ?  , link=? where name = ?");
  $stmt->execute([$name, $description, $price , $categorie , $image, $autheur , $date , $created , $fileName , $name]);
